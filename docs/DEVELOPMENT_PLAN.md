...
| Styling           | Plain CSS with warm compact semantic design tokens; no full UI framework                                                                       |
...
The container sets `IIU_NAV_DATA_DIR=/data`. Direct local runs default to `./data`; the environment variable can select another root. `/data/tmp` hosts process temporary files such as multipart upload spooling: the server redirects the process temp directory there at startup so large import and restore uploads never depend on the small container tmpfs. Startup also reconciles data roots left inconsistent by a process crash during a restore swap. Database timestamps are stored as UTC Unix milliseconds and schema tables use SQLite `STRICT` typing.
...
| `NAV-418` | `DONE` | `NAV-305`, `NAV-417`            | Restricted Docker host executor and administrator one-click update workflow                                                    | The Docker update criteria in `FR-019` pass, including password reverification, pre-update backup, digest verification, temporary-outage progress, health verification, schema-aware failure recovery, preserved `/data`, and denial of arbitrary Docker or host operations                                                                                                                                                                                                     | The executor is optional and Linux Docker Compose specific; installations without it retain the manual update path; never mount the Docker socket into the application container Shipped as `internal/hostexecutor` (fixed update/status protocol; release identity and digest verified independently against the official GitHub manifest; digest-pinned pull; executor-owned compose override file; `up --wait` health gate; rollback to the previous image and, when the old binary refuses an advanced schema, automatic pre-update-backup restore after stopping the service) running in a `docker:cli`-based executor container published by the release workflow; `cmd/iiiu-nav restore <archive>` provides the container-side recovery primitive; the application gains `POST /api/updates/install` (password reverification, forced re-check pinning the target to the official latest stable, consistent pre-update backup) and `GET /api/updates/install/status` behind `internal/updateexecutor`, with the browser keeping a progress view across the brief restart; `compose.executor.yaml` installs the executor without granting the application container the Docker socket; OPERATIONS 7.1 documents install, flow, and removal; hostexecutor release-validation/rollback/backup-recovery unit tests, updateexecutor socket-client tests, server install-flow integration tests, and 108 frontend tests passed with all verification gates on 2026-09-18 |
| `NAV-419` | `DONE` | `NAV-406`                       | Direct administrator entry for authenticated public sessions                                                                   | The public page keeps public-only content and controls; when the current session is authenticated, every administrator entry navigates directly to `/admin`, while anonymous sessions navigate to `/admin/login`, without briefly rendering the login page                                                                                                                                                                                                                      | The public-scope response already exposed authenticated session state without private content, so the fixed `/admin/login` destination was replaced with a session-aware `/admin` or `/admin/login` destination; administrator entries now use semantic links with the shared button visual primitive while the public page continues to suppress management controls; regression coverage verifies both destinations and the retained public-only interface; 27 frontend files / 91 tests, all Go tests, `go vet`, formatting, lint, TypeScript, and production build passed on 2026-08-20                                                                                                                                                                                                                                                                                                                                |
| `NAV-420` | `DONE` | `NAV-414`, `NAV-419`            | Warm compact design-system alignment                                                                                           | The existing Preact and plain-CSS UI absorbs the accepted warm compact palette, spacing, radius, density, table, and responsive rules without adding Ant Design or React; the public and administrator sidebars retain distinct hierarchy, configurable site accent and light/dark/system themes continue to work, business controls use a compact consistent height while the login remains touch-comfortable, and layouts remain overflow-safe at 360, 768, 1024, and 1440 px | Warm canvas/sidebar/surface/text and graphite dark-mode tokens now follow the accepted system; configurable accents remain contrast-mapped with `#305880` as the fresh-install default, high-contrast status colors are retained, business controls use 28/32/36 px density with a 44 px login exception and a coarse-pointer small-control floor, the public sidebar uses the secondary warm surface, and the administrator uses a 216 px blue-gray operations shell with compact tables and toolbars; no Ant Design, React, Less, or new dependency was added; 27 frontend files / 91 tests, all Go tests, `go vet`, formatting, lint, TypeScript, production build, computed color checks, and headless Chrome review of public, login, dashboard, and link-management surfaces in light/dark modes without page overflow at 360/768/1024/1440 px passed on 2026-08-20                                                  |
| `NAV-421` | `DONE` | `NAV-417`                       | Review hardening: large-upload spooling, interrupted-restore recovery, stacked-modal Escape, long-request timeouts, address-aware healthcheck, and 5xx logging                                 | Deep-review findings are fixed with regression coverage: the documented restore and import size limits are achievable in the hardened container, a client disconnect or crash during restore cannot leave the store closed or the data root without a database, stacked overlays close only the top layer, long operations stream past the 30-second write timeout, and the container healthcheck follows `IIU_NAV_ADDR`                                                                                                                                                                                                                                                                                                                                                                                                                                               | Process temp files now spool under `/data/tmp` through the redirected process temp directory; startup reconciliation removes pre-swap `.restore-validating-*` workspaces and restores the previous database and uploads from a `.restore-rollback-*` directory when the active database is missing, leaving leftovers in place with a logged notice otherwise; restore runs on an uncanceled request context; restore, import, backup creation, and bulk refresh clear per-request deadlines; Escape closes only the most recently opened overlay; the healthcheck derives its target from `IIU_NAV_ADDR`; server warnings now record 5xx method, path, and status; 28 frontend files / 92 tests, all Go tests, `go vet`, formatting, lint, TypeScript, and production build passed on 2026-09-16                                                                                                                           |
| `NAV-422` | `DONE` | `NAV-421`                                  | Second hardening pass: health availability during restore, bounded background refresh, sentinel-based error mapping, atomic ordering, session sweeps, broad platform disk checks, app-wide accent application, request races, i18n aria labels, and a push-triggered CI workflow | All remaining review findings are fixed: `/healthz` and `/api/health` stay reachable while a restore holds the maintenance gate, post-import metadata refresh derives from a cancellable background context, link and category ordering is assigned inside the create transaction with a category-existence check, import size and option errors use sentinels instead of message matching, expired sessions are swept on login, disk-space checks cover darwin and the BSDs, background uploads are bounded to 16M pixels, image-cleanup failures are logged, and the accent color re-derives design tokens app-wide from the document root | Also fixed an accent defect found during re-verification: the previous subtree override never re-derived the `:root` accent tokens, so customized accents were silently ignored on every surface; `--site-accent` is now set on the document root. Frontend races (version check, backup listing) are sequenced, backup and import responses are validated, four hardcoded aria labels moved into the message catalog, and the dead mock-data label was removed. Pointer-drag glue is covered by driving the production monitor callback; real pointer interaction still requires a browser. CI runs frontend checks, production build, gofmt, Go tests, `go vet`, and the container go-test stage on every push and pull request; 30 frontend files / 95 tests, all Go tests, cross-builds for linux, windows, darwin, freebsd, openbsd, and dragonfly, and production build passed on 2026-09-16  |
| `NAV-423` | `DONE` | `NAV-422`                                  | UI/UX accessibility and interaction pass: skip links, private-category labels, touch targets, background storage size, text contrast, administrator deep links, back-key overlay handling, and chart data access | All nine reviewed UI/UX findings are fixed: the public page and administrator workspace expose skip-to-content links, private categories announce through visually hidden text instead of an SVG label, high-frequency touch controls rise to 40px on coarse pointers, link logos lazy-load, opaque backgrounds store as JPEG (quality 82) with transparency still stored as PNG and restore/prune policies accepting both, auxiliary light-theme text meets WCAG AA on every surface, administrator modules deep-link through `/admin/<section>` URLs, the browser back key closes the topmost overlay, and the visitor chart exposes per-day values on hover/focus plus a visually hidden data table | The background change supersedes the implicit all-PNG storage with JPEG only where an image is fully opaque; existing PNG assets, restore validation, and pruning accept both extensions. The coarse-pointer floor supersedes the 32px small-control floor for high-frequency controls (category chips, sort handles and arrows, row actions) at 40px. Light-theme `--color-text-muted` deepened to `#63615b` and `--color-text-subtle` to `#6f6d66` after re-measurement showed the previous subtle tone at 3.03:1 on the warm canvas; dark theme tones already passed. 30 frontend files / 98 tests, all Go tests with new imagestore and restore policy coverage, `go vet`, formatting, lint, TypeScript, and production build passed on 2026-09-16 |
| `NAV-424` | `DONE` | `NAV-422`                                  | Architecture deepening pass: production background-context wiring with ordered shutdown, history-state preservation on section switches, and one reference-aware image cleanup module | The server `Background` seam is honored in production: the composition root supplies it, shutdown drains in-flight requests, cancels background work, and closes the data store only afterward; administrator section switches preserve the browser history state object instead of erasing overlay sentinels; and link, category, and metadata logo retirement all run through the `internal/imagecleanup` module, which validates path ownership through the store and logs counting and removal failures | This also completes the NAV-422 intent that cleanup failures be logged: that fix had landed only on the site-settings path, while the link, category, and metadata retirement sites silently dropped errors. `shutdownServer` now completes before `serve` returns, so the deferred database close no longer races connection drain. Replace-semantics site image cleanup stays with the site-settings handler (single caller, retain-set policy, already logged). Six imagecleanup unit tests plus an admin-page history-state regression test were added; 31 frontend files / 99 tests, all Go tests, `go vet`, formatting, lint, TypeScript, and production build passed on 2026-09-18 |
...
- The default accent color for new installations is the warm compact primary `#305880`; existing saved site accents remain unchanged.
...
| Version | Date         | Change                                                                                                                                                                                                                                      |
| ------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `1.44`  | `2026-09-18` | One-click updates: restricted Docker host executor with manifest-verified digest updates, health-gated swap, schema-aware rollback with backup restore, guarded administrator install flow with progress view, and a restore CLI subcommand |
| `1.43`  | `2026-09-18` | Deepening pass: wired the production background context with ordered drain-then-close shutdown, preserved history state on administrator section switches, and unified reference-aware image cleanup with complete failure logging in one module |
| `1.42`  | `2026-09-16` | UI/UX accessibility pass: skip links, reliable private-category announcements, 40px coarse-pointer touch targets, JPEG storage for opaque backgrounds, WCAG AA auxiliary text contrast, administrator deep links, back-key overlay closing, and chart data tables |
| `1.41`  | `2026-09-16` | Fixed all remaining review findings: health availability during restores, cancellable import enrichment, sentinel error mapping, transactional ordering, login-time session sweeps, darwin/BSD disk checks, sequenced frontend requests, validated import and backup responses, centralized aria labels, push-triggered CI, and an app-wide accent fix that also repaired silently ignored custom accents |
| `1.40`  | `2026-09-16` | Hardened large-upload spooling onto the data volume, added interrupted-restore startup reconciliation and disconnect-safe restore execution, fixed stacked-overlay Escape handling and long-request timeouts, made the healthcheck address-aware, and added 5xx request logging |
| `1.39`  | `2026-08-20` | Aligned warm compact semantic colors, density, sidebars, tables, responsive behavior, dark mode, and the fresh-install accent default while preserving the lightweight Preact and plain-CSS architecture                                    |
| `1.38`  | `2026-08-20` | Routed authenticated public-page administrator entries directly to `/admin` while preserving `/admin/login` for anonymous sessions and keeping the public interface free of management controls                                             |
